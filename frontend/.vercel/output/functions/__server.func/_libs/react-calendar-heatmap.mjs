...
//#region node_modules/react-calendar-heatmap/dist/react-calendar-heatmap.esm.js
...
var factoryWithThrowingShims = function() {
	function shim(props, propName, componentName, location, propFullName, secret) {
		if (secret === ReactPropTypesSecret_1) return;
...
var MILLISECONDS_IN_ONE_DAY = 1440 * 60 * 1e3;
var DAYS_IN_WEEK = 7;
var MONTH_LABELS = [
	"Jan",
	"Feb",
...
function getRange(count) {
	var arr = [];
	for (var idx = 0; idx < count; idx += 1) arr.push(idx);
	return arr;
}
var SQUARE_SIZE = 10;
	...
	_createClass(CalendarHeatmap, [
		...
		},
		{
			...
			value: function getMonthLabelSize() {
				if (!this.props.showMonthLabels) return 0;
				if (this.props.horizontal) return SQUARE_SIZE + MONTH_LABEL_GUTTER_SIZE;
		...
		},
		{
			...
			value: function getWeekdayLabelSize() {
				if (!this.props.showWeekdayLabels) return 0;
				if (this.props.horizontal) return 30;
		...
		},
		{
			...
			value: function getValueForIndex(index) {
				if (this.valueCache[index]) return this.valueCache[index].value;
				return null;
			}
		},
		{
			...
			value: function getClassNameForIndex(index) {
				if (this.valueCache[index]) return this.valueCache[index].className;
		...
		},
		{
			...
			value: function getTitleForIndex(index) {
				if (this.valueCache[index]) return this.valueCache[index].title;
		...
		},
		{
			...
			value: function getTooltipDataAttrsForIndex(index) {
				if (this.valueCache[index]) return this.valueCache[index].tooltipDataAttrs;
		...
		},
		{
			...
			value: function getTransformForWeek(weekIndex) {
				if (this.props.horizontal) return "translate(".concat(weekIndex * this.getSquareSizeWithGutter(), ", 0)");
		...
		},
		{
			...
			value: function getTransformForWeekdayLabels() {
				if (this.props.horizontal) return "translate(".concat(SQUARE_SIZE, ", ").concat(this.getMonthLabelSize(), ")");
				return null;
			}
		},
		{
			...
			value: function getTransformForMonthLabels() {
				if (this.props.horizontal) return "translate(".concat(this.getWeekdayLabelSize(), ", 0)");
		...
		},
		{
			...
			value: function getTransformForAllWeeks() {
				if (this.props.horizontal) return "translate(".concat(this.getWeekdayLabelSize(), ", ").concat(this.getMonthLabelSize(), ")");
		...
		},
		{
			...
			value: function getViewBox() {
				if (this.props.horizontal) return "0 0 ".concat(this.getWidth(), " ").concat(this.getHeight());
		...
		},
		{
			...
			value: function getSquareCoordinates(dayIndex) {
				if (this.props.horizontal) return [0, dayIndex * this.getSquareSizeWithGutter()];
		...
		},
		{
			...
			value: function getWeekdayLabelCoordinates(dayIndex) {
				if (this.props.horizontal) return [0, (dayIndex + 1) * SQUARE_SIZE + dayIndex * this.props.gutterSize];
		...
		},
		{
			...
			value: function getMonthLabelCoordinates(weekIndex) {
				if (this.props.horizontal) return [weekIndex * this.getSquareSizeWithGutter(), this.getMonthLabelSize() - MONTH_LABEL_GUTTER_SIZE];
				return [0, (weekIndex + 1) * this.getSquareSizeWithGutter() + -2];
			}
		},
		{
			key: "handleClick",
		...
		},
		{
			...
			value: function handleMouseOver(e, value) {
				if (this.props.onMouseOver) this.props.onMouseOver(e, value);
			}
		},
		{
			key: "handleMouseLeave",
			value: function handleMouseLeave(e, value) {
				if (this.props.onMouseLeave) this.props.onMouseLeave(e, value);
			}
		},
		{
			key: "renderSquare",
			value: function renderSquare(dayIndex, index) {
				var _this2 = this;
				if ((index < this.getNumEmptyDaysAtStart() || index >= this.getNumEmptyDaysAtStart() + this.getDateDifferenceInDays()) && !this.props.showOutOfRangeDays) return null;
				var _this$getSquareCoordi2 = _slicedToArray(this.getSquareCoordinates(dayIndex), 2), x = _this$getSquareCoordi2[0], y = _this$getSquareCoordi2[1];
				var value = this.getValueForIndex(index);
				var rect = import_react.createElement("rect", _extends({
					key: index,
					width: SQUARE_SIZE,
					height: SQUARE_SIZE,
					x,
				...
				}, this.getTooltipDataAttrsForIndex(index)), import_react.createElement("title", null, this.getTitleForIndex(index)));
		...
		},
		{
			...
			value: function renderMonthLabels() {
				...
				return getRange(this.getWeekCount() - 1).map(function(weekIndex) {
					var endOfWeek = shiftDate(_this5.getStartDateWithEmptyDays(), (weekIndex + 1) * DAYS_IN_WEEK);
					var _this5$getMonthLabelC2 = _slicedToArray(_this5.getMonthLabelCoordinates(weekIndex), 2), x = _this5$getMonthLabelC2[0], y = _this5$getMonthLabelC2[1];
					return endOfWeek.getDate() >= 1 && endOfWeek.getDate() <= DAYS_IN_WEEK ? import_react.createElement("text", {
						key: weekIndex,
						x,
						y,
		...
		},
		{
			...
			value: function renderWeekdayLabels() {
				var _this6 = this;
				if (!this.props.showWeekdayLabels) return null;
				return this.props.weekdayLabels.map(function(weekdayLabel, dayIndex) {
					var _this6$getWeekdayLabe2 = _slicedToArray(_this6.getWeekdayLabelCoordinates(dayIndex), 2), x = _this6$getWeekdayLabe2[0], y = _this6$getWeekdayLabe2[1];
					...
					return dayIndex & 1 ? import_react.createElement("text", {
		...
		},
		{
			...
			value: function render() {
				...
				}, this.renderMonthLabels()), import_react.createElement("g", {
					transform: this.getTransformForAllWeeks(),
					className: "".concat(CSS_PSEDUO_NAMESPACE, "all-weeks")
				}, this.renderAllWeeks()), import_react.createElement("g", {
...
CalendarHeatmap.propTypes = {
	values: propTypes.arrayOf(propTypes.shape({ date: propTypes.oneOfType([
		propTypes.string,
		propTypes.number,
